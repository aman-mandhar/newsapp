    ...
    <form action="{{ route('admin.posts.update', $post) }}" method="POST" enctype="multipart/form-data" class="border p-4 rounded shadow-sm">
        ...
        <div class="row mb-3">
            ...
            <div class="col-md-6 mb-3">
                <label class="form-label fw-bold" for="pdfUrl">PDF URL (Google Drive/Direct Link)</label>
                <input type="url"
                    id="pdfUrl"
                    name="pdf_url"
                    class="form-control @error('pdf_url') is-invalid @enderror"
                    value="{{ old('pdf_url', $post->pdf_url) }}"
                    placeholder="https://drive.google.com/file/d/...">
                @error('pdf_url') <small class="text-danger">{{ $message }}</small> @enderror
                <small class="text-muted">Paste unrestricted Google Drive or direct PDF link</small>
            </div>
            <div class="col-md-6 mb-3">
                <label class="form-label fw-bold" for="pdfTitle">PDF Title (Optional)</label>
                <input type="text"
                    id="pdfTitle"
                    name="pdf_title"
                    class="form-control @error('pdf_title') is-invalid @enderror"
                    value="{{ old('pdf_title', $post->pdf_title) }}"
                    placeholder="e.g., Download Full Report">
                @error('pdf_title') <small class="text-danger">{{ $message }}</small> @enderror
                <small class="text-muted">Display name for the PDF section</small>
            </div>
        </div>
